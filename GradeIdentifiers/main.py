...
from schema import GradeIdentifiersRow
...
coders.registry.register_coder(GradeIdentifiersRow, coders.RowCoder)
...
def run():
    ...
    google_cloud_options.job_name = 'bq-to-postgres-identifiers'
    ...
    bq_table = "grade_identifiers"
    schema_name="grade"
    last_id = read_last_id_from_postgres()

    with beam.Pipeline(options=google_cloud_options) as pipeline:
        from apache_beam.io.jdbc import WriteToJdbc
        from schema import GradeIdentifiersRow,dict_to_grade_identifiers_row , convert_to_utc_timestamp
        ...
        rows = (
            pipeline
                ...
                | f"ConvertToGradeIdentifiersRow" >> beam.Map(lambda row: dict_to_grade_identifiers_row(row, GradeIdentifiersRow)).with_output_types(GradeIdentifiersRow)
